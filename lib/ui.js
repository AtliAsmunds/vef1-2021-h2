...
function renderTaskList(sortKey = 'due') {
  ...
  if (params.get('category')) {
    tasks = [];
    let catEl = document.querySelector('#' + params.get('category'));
    ...
    tasks = [];
    let tagEl = document.querySelector('#' + params.get('tag'));
    ...
    if(params.get('completed')==='true') {
      let tagEl = document.querySelector('#finished');
      tagEl.classList.add('menu-item-selected');
    } else {
      let tagEl = document.querySelector('#unfinished');
      tagEl.classList.add('menu-item-selected');
    }
    for (const task of data.items) {
    ...
    let tagEl = document.querySelector('#all-tasks');
    tagEl.classList.add('menu-item-selected');
    tasks = data.items;
  }
...
function handleNewTaskClick(modify, id) {
  return (e) => {
    ...
    let warningString = '';
    const data = JSON.parse(localStorage.getItem('data'));
    let titill = document.querySelector('#titill').value;
    if(titill === '') warningString = 'Verkefnið þarf að hafa titil\n';
    let lysing = document.querySelector('#lysing').value;
    ...
    let ja = document.querySelector('#jaForgang').checked;
    let nei = document.querySelector('#neiForgang').checked;
    let priority = false;
    let completed = false;
    ...
    if (modify) {
      for (const tag of data.items[ID - 1].tags) {
        console.log(tag);
        data.taskPerTag[tag]--;
        console.log(data.taskPerTag[tag]);
      }
    }
